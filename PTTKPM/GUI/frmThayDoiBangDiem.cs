    ...
    public partial class frmThayDoiBangDiem : DevComponents.DotNetBar.Office2007Form
    {
        ...
        private void XL_Chon()
        {
            if (flag == true)
            {
                ...
                HocSinhBUS adapterHS = new HocSinhBUS();
                if (combHK.Text != "" && combLop.Text != "" && combMonHoc.Text != "" && combHK.SelectedIndex >= 0 && combLop.SelectedIndex >= 0 && combMonHoc.SelectedIndex>=0)
                {
                    ...
                    if (chi_soHK >= 0 && chi_soLop >= 0 && chi_soMH >= 0)
                    {
                        ...
                        HocSinh[] HS = adapterHS.getAllStudentIf_BUS(KQL.MaLop, KQMH.MaMonHoc, KQHK.MaHocKi);

                        diem = adapterDiem.getMarkIf_BUS(KQL.MaLop, KQMH.MaMonHoc, KQHK.MaHocKi);
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            ...
            DiemBUS adapterDiem = new DiemBUS();
            if (adapterDiem.UpdateMark_BUS(diem))
            {
